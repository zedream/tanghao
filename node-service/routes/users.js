...
router.post('/modifyPassword', function(req, res, next) {
  ...
  db.query('update user set password="'+ password +'" where username="'+ username +'"', [], (result, fields) => {
    let data = {
      code: 0,
      msg: '修改成功'
    }
...
router.get('/allInfo', function(req, res, next) {
  ...
  let results = {}
  db.query('select * from user where username="'+ username +'"', [], (result, fields) => {
    Object.assign(results, result[0])
    db.query('select * from extrainfo where username="'+ username +'"', [], (result, fields) => {
      Object.assign(results, result[0])
      db.query('select * from project where username="'+ username +'"', [], (result, fields) => {
        let temp = []
...
router.post('/personal', function(req, res, next) {
  ...
  db.query('update user set nick_name="'+ nickName +'", age="'+ age +'", hometown="'+ hometown +'", nation="'+ nation +'", residence="'+ residence +'" where username="'+ username +'"', [], (result, fields) => {
    let data = {
      code: 0,
      msg: '保存成功'
    }
...
router.post('/education', function(req, res, next) {
  ...
  db.query('update extrainfo set education="'+ education +'", study_years="'+ studyYears +'", major="'+ major +'", school="'+ school +'" where username="'+ username +'"', [], (result, fields) => {
    let data = {
      code: 0,
      msg: '保存成功'
    }
...
router.post('/company', function(req, res, next) {
  ...
  db.query('update extrainfo set company="'+ company +'", work_years="'+ workYears +'", position="'+ position +'" where username="'+ username +'"', [], (result, fields) => {
    let data = {
      code: 0,
      msg: '保存成功'
    }
...
router.post('/project', function(req, res, next) {
  ...
  console.log(name)
  db.query('update project set name="'+ name.join(',') +'", period="'+ period.join(',') +'", duty="'+ duty.join(',') +'", work_content="'+ workContent.join(',') +'" where username="'+ username +'"', [], (result, fields) => {
    let data = {
      code: 0,
      msg: '保存成功'
    }
